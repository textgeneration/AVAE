...
def main():
    ...
    for it in tqdm(range(n_iter)):
        ...
        recon_loss, kl_loss, clf_loss= model.forward(inputs, labels, use_c_prior=False)
        ...
        loss_vae = recon_loss + kl_weight_max * kl_loss + 0.2*clf_loss
        loss_attr_c = F.cross_entropy(y_c, target_c)
        loss_attr_z = F.mse_loss(y_z, target_z)

        loss_G = lambda_c*loss_attr_c + lambda_z*loss_attr_z
        ...
        #Update encoder, eq. 4 
        recon_loss, kl_loss, clf_loss = model.forward(inputs, labels, use_c_prior=True)

        loss_E = recon_loss + kl_weight_max * kl_loss + + 0.2*clf_loss
        ...
        if it % log_interval == 0:
            ...
            print('Iter-{}; loss_D: {:.4f}; loss_G: {:.4f}; loss_attr_c: {:.4f}; loss_attr_z: {:.4f}; loss_vae: {:.4f}'
                  .format(it, float(loss_D), float(loss_G), float(loss_attr_c), float(loss_attr_z), float(loss_vae)))
...
def save_model():
    ...
    torch.save(model.state_dict(), 'models/AVae.bin')
